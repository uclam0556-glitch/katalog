...
const { showToast } = useToast(); // Assuming standard toast hook exists or will use simple alert/custom toast
    ...
    if (navigator.share) {
        try {
            await navigator.share({
                title: product.name,
                text: `Посмотрите ${product.name} в амэа`,
                url: url,
            });
        } catch (error) {
        ...
        try {
            ...
            alert("Ссылка скопирована!"); // Simple fallback if no toast
        } catch (err) {
